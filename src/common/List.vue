<template>
  <div class="list">
    <template v-if="mold === 'thumbnail'" v-for="(item,index) in items">
      <router-link class="thumbnail" :key="index" :to="{name:'DetailView',params: { id : item.id}}">
        <div class="content">
        ...
        <div class="author">
          ...
          <span class="label" v-if="item.subcategory_name">本活动来自栏目 {{item.subcategory_name}}</span>
        </div>
      </router-link>
    </template>
